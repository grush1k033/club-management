    ...
    public function getPlatformStats() {
        try {
            ...
            SELECT 
                ...
                (SELECT COUNT(*) FROM users u WHERE u.club_id = c.id AND u.is_active = 1) AS member_count,
                
                -- Количество ивентов клуба
            ...
            FROM clubs c
            LEFT JOIN users captain 
                ON captain.id = c.captain_id 
            LEFT JOIN users vice_captain 
                ON vice_captain.id = c.vice_captain_id
            ORDER BY c.name ASC
        ";
            ...
            foreach ($clubs as &$club) {
                $club['club_id'] = (int)$club['club_id'];
                $club['member_count'] = (int)$club['member_count'];
                $club['event_count'] = (int)$club['event_count'];
            }

            Response::success('Клубы со статистикой успешно получены', $clubs);

        } catch (Exception $e) {
            Response::error('Ошибка при получении данных клубов: ' . $e->getMessage(), [], 500);
        }
    }
